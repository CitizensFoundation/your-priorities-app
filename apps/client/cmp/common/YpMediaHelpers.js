export class YpMediaHelpers {
    static _checkVideoLongPlayTimeAndReset(playbackElement, videoPlayer) {
        const videoId = videoPlayer.getAttribute('data-id');
        if (playbackElement.playStartedAt && videoId) {
            const seconds = (new Date().getTime() - playbackElement.playStartedAt.getTime()) / 1000;
            if (seconds > 5) {
    ...
    static _checkAudioLongPlayTimeAndReset(playbackElement, audioPlayer) {
    ...
    static getImageFormatUrl(images, formatId = 0) {
    ...
    // TODO: Test this well
    static attachMediaListeners(targetElement) {
        setTimeout(() => {
            ...
            if (videoPlayer) {
                ...
                if (videoId) {
                    ...
                    videoPlayer.addEventListener('pause', targetElement.videoPauseListener.bind(targetElement));
                    videoPlayer.addEventListener('ended', targetElement.videoEndedListener.bind(targetElement));
                }
            }
            if (audioPlayer) {
                const audioId = audioPlayer.getAttribute('data-id');
                if (audioId) {
                    ...
                    audioPlayer.addEventListener('pause', targetElement.audioPauseListener.bind(targetElement));
                    audioPlayer.addEventListener('ended', targetElement.audioEndedListener.bind(targetElement));
                }
            }
        }, 200);
    }
    static detachMediaListeners(targetElement) {
        const videoPlayer = targetElement.$$('#videoPlayer');
        const audioPlayer = targetElement.$$('#audioPlayer');
        if (videoPlayer) {
            if (targetElement.videoPlayListener) {
                videoPlayer.removeEventListener('play', targetElement.videoPlayListener);
                targetElement.videoPlayListener = undefined;
            }
            if (targetElement.videoPauseListener) {
                videoPlayer.removeEventListener('pause', targetElement.videoPauseListener);
                targetElement.videoPauseListener = undefined;
            }
            if (targetElement.videoEndedListener) {
                videoPlayer.removeEventListener('ended', targetElement.videoEndedListener);
    ...
    static isPortraitVideo(videos) {
        if (videos &&
            videos.length > 0 &&
            videos[0].formats &&
            videos[0].formats.length > 0) {
            if (videos[0].public_meta &&
    ...
    static getVideoPosterURL(videos, images, selectedImageIndex = 0) {
        if (videos &&
            videos.length > 0 &&
...
//# sourceMappingURL=YpMediaHelpers.js.map