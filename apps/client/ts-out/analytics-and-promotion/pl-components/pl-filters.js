...
import { FILTER_GROUPS, formatFilterGroup, filterGroupForFilter, toFilterType, valueWithoutPrefix, } from './util/filter.js';
...
let PlausibleFilters = class PlausibleFilters extends PlausibleBaseElementWithState {
    constructor() {
        super(...arguments);
        this.viewport = 1080;
        this.wrapped = 1; // 0=unwrapped, 1=waiting to check, 2=wrapped
    ...
    updated(changedProperties) {
        ...
        if (changedProperties.get('wrapped') &&
            this.wrapped === 1 &&
    ...
    removeFilter(key) {
        ...
        this.menuOpen = false;
        navigateToQuery(this.history, this.query, newOpts);
    }
    clearAllFilters() {
        const newOpts = Object.keys(this.query.filters).reduce((acc, red) => ({ ...acc, [red]: false }), {});
        navigateToQuery(this.history, this.query, newOpts);
        this.menuOpen = false;
    }
    filterText(key, rawValue) {
      ...
      <div key=${key}>
        ...
        >
          <pl-link
            ...
            .to=${{
            pathname: `/${encodeURIComponent(this.site.domain)}/filter/${filterGroupForFilter(key)}`,
    ...
    filterDropdownOption(option) {
      ...
      <div key=${option}>
        <pl-link
          .to=${{
            pathname: `/${encodeURIComponent(this.site.domain)}/filter/${option}`,
            search: window.location.search,
        }}
          class="${active
    ...
    renderDropdownContentOriginal() {
        if (this.wrapped === 0 || this.addingFilter) {
            return Object.keys(FILTER_GROUPS)
                .filter(option => option === 'props' ? this.site.flags.custom_dimension_filter : true)
        ...
        ${appliedFilters(this.query).map(filter => this.renderDropdownFilter(filter))}
        <div key="clear">
          <div
    ...
    renderFilterList() {
        ...
        <div id="filters" class="flex flex-wrap">
          ${appliedFilters(this.query).map(filter => this.renderListFilter(filter))}
        </div>
      `;
        }
        else {
...
], PlausibleFilters.prototype, "url", void 0);
__decorate([
    property({ type: Number })
], PlausibleFilters.prototype, "viewport", void 0);
__decorate([
    property({ type: Number })
], PlausibleFilters.prototype, "wrapped", void 0);
__decorate([
    property({ type: Object })
], PlausibleFilters.prototype, "history", void 0);
__decorate([
    property({ type: Boolean })
], PlausibleFilters.prototype, "addingFilter", void 0);
__decorate([
    property({ type: Boolean })
], PlausibleFilters.prototype, "menuOpen", void 0);
PlausibleFilters = __decorate([
    customElement('pl-filters')
], PlausibleFilters);
export { PlausibleFilters };
//# sourceMappingURL=pl-filters.js.map