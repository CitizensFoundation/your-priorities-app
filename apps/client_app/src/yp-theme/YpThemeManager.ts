export class YpThemeManager {
  ...
  constructor() {
    ...
    this.themes.push({
      ...
      '--primary-color': '#7dbdc9',
      '--accent-color': '#d0672f',
      fonts: { htmlImport: "/styles/fonts/forbrukerradet-font.html", fontName: 'FFClanWebBook' }
    });
  ...
  private _setupOverrideTheme(primary: string, accent: string, background: string) {
    if (!background) {
  ...
  private _updateStyles(element: HTMLElement, properties: Record<string,string>) {
  ...
  private _onlyGetStylesFromTheme(properties: Record<string,boolean|string|Record<string,string>>) {
  ...
  setTheme(number: number, element: HTMLElement, configuration: YpCollectionConfiguration) {
    if (configuration && configuration.themeOverrideColorPrimary &&
      ...
      this._updateStyles(element, this._setupOverrideTheme(configuration.themeOverrideColorPrimary, configuration.themeOverrideColorAccent, configuration.themeOverrideBackgroundColor))
    } else {
      if (this.themes[number]) {
        this._updateStyles(element, this._onlyGetStylesFromTheme(this.themes[number]))
      }
    }
  }
}