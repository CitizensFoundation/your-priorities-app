...
export class YpPostBaseWithAnswers extends YpBaseElement {
  @property({ type: Array })
  ...
  async _getSurveyTranslationsIfNeeded() {
    ...
    if (this.post &&
      ...
      const questionsIndexKey = this._getIndexTranslationKey("GroupQuestions");
      const answersIndexKey = this._getIndexTranslationKey("PostAnswers");
      if (window.appGlobals.cache.autoTranslateCache[questionsIndexKey] &&
        ...
        this.translatedQuestions = window.appGlobals.cache.autoTranslateCache[questionsIndexKey] as string[];
        this.translatedAnswers = window.appGlobals.cache.autoTranslateCache[answersIndexKey] as string[];
      } else {

        const translatedResponse = await window.serverApi.getSurveyTranslations(this.post, this.language) as string[][];
        ...
        if (this.post &&
          ...
          if (translatedQuestionTexts.length===YpSurveyHelpers.getQuestionLengthWithSubOptions(currentQuestions)) {
            ...
            window.appGlobals.cache.autoTranslateCache[questionsIndexKey] = currentQuestions;
            window.appGlobals.cache.autoTranslateCache[answersIndexKey] = currentAnswers;
  ...
  _getIndexTranslationKey(textType: string) {
