...
const aboutPrompt = new PromptTemplate({
  template: aboutPromptTemplate,
  input_variables: [],
});

function getAboutProjectPrompt(question: string): ChatPromptTemplate {
  const messages = [
    SystemMessagePromptTemplate.fromTemplate(aboutPromptTemplate),
