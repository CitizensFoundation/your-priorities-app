var filterNotificationForDelivery =
  require("./emails_utils.cjs").filterNotificationForDelivery;
...
module.exports = function (notification, user, callback) {
  if (
    ...
    if (notification.type == "notification.point.new") {
      ...
      var subjectTranslateToken =
        notification.AcActivities[0].type.indexOf("activity.point.helpful") > -1
      ...
      filterNotificationForDelivery(
        ...
        user,
        "point_activity",
        { translateToken: subjectTranslateToken, contentName: contentName },
        callback
      );
    } else {
      callback(
        "Unknown state for notification filtering: " + notification.type
      );
    }
  } else {
