        ...
        models.Domain.findOne({
            ...
            if (domain) {
                ...
                models.Community.findAndCountAll({
                    ...
                    const sharingParameters = await getSharingParameters(req, domain, getFullUrl(req), imageUrl);
                    var botOptions = {
...
const completeSendingCommunity = async (community, req, res) => {
    ...
    const sharingParameters = await getSharingParameters(req, community, getFullUrl(req), imageUrl);
    var botOptions = {
        url: sharingParameters.url,
        title: sharingParameters.title,
        descriptionText: sharingParameters.description,
...
const completeSendingGroup = async (group, postsInfo, postsOffset, req, res) => {
    ...
    const sharingParameters = await getSharingParameters(req, group, getFullUrl(req), imageUrl);
    const botOptions = {
        url: sharingParameters.url,
        title: sharingParameters.title,
        descriptionText: sharingParameters.description,
