...
const importPost = require('./aiAssistantApi').importPost;
const importPoint = require('./aiAssistantApi').importPoint;
...
const updateCollection = (workPackage, done) => {
    if (process.env.AC_AI_ASSISTANT_KEY &&
        process.env.AC_AI_ASSISTANT_CLUSTER_ID &&
        process.env.AC_AI_ASSISTANT_BASE_URL) {
        ...
        log.warn("Can't find AC_AI_ASSISTANT_KEY, AC_AI_ASSISTANT_CLUSTER_ID & AC_AI_ASSISTANT_BASE_URL for the similarities engine");
        done();
    }
};
const getFromAnalyticsApi = (req, featureType, collectionType, collectionId, done) => {
    ...
    req.redisClient.get(redisKey, (error, content) => {
        if (!error && content) {
            ...
            const options = {
                url: process.env["AC_AI_ASSISTANT_BASE_URL"] + featureType + "/" + collectionType + "/" + process.env.AC_AI_ASSISTANT_CLUSTER_ID + "/" + collectionId,
                headers: {
...
const triggerSimilaritiesTraining = (req, collectionType, collectionId, done) => {
    const options = {
        url: process.env["AC_AI_ASSISTANT_BASE_URL"] + "trigger_similarities_training" + "/" + collectionType + "/" + process.env.AC_AI_ASSISTANT_CLUSTER_ID + "/" + collectionId,
        headers: {
            'X-API-KEY': process.env["AC_AI_ASSISTANT_KEY"]
        },
        json: {}
...
const convertDocxSurveyToJson = (formData, done) => {
    const options = {
        url: process.env["AC_AI_ASSISTANT_BASE_URL"] + "convert_doc_x_survey_to_json",
        headers: {
            'X-API-KEY': process.env["AC_AI_ASSISTANT_KEY"]
        },
        formData: formData,
