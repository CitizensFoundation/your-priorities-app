...
class InterviewVectorStore extends base_js_1.Base {
    ...
    async addSchema() {
        let classObj;
        try {
            const data = await promises_1.default.readFile("./schemas/chatBotConfig.json", "utf8");
        ...
        try {
            const res = await InterviewVectorStore.client.schema
                .classCreator()
                .withClass(classObj)
                .do();
            console.log(res);
    ...
    async showScheme() {
        try {
            const res = await InterviewVectorStore.client.schema.getter().do();
    ...
    async testQuery() {
        ...
        const res = await InterviewVectorStore.client.graphql
            .get()
            .withClassName("WebPage")
            .withWhere({
            operator: "And",
    ...
    async postWebPage(webPageAnalysis) {
        return new Promise((resolve, reject) => {
            InterviewVectorStore.client.data
                .creator()
                .withClassName("WebPage")
    ...
    async getWebPagesForProcessing(groupId, subProblemIndex = undefined, entityIndex = undefined, searchType, limit = 10, offset = 0, solutionCountLimit = 0) {
        ...
        let query;
        try {
            query = InterviewVectorStore.client.graphql
                .get()
                .withClassName("WebPage")
                .withLimit(limit)
                .withOffset(offset)
    ...
    async webPageExist(groupId, url, searchType, subProblemIndex, entityIndex) {
        ...
        let results;
        try {
            results = await InterviewVectorStore.client.graphql
                .get()
                .withClassName("WebPage")
                .withLimit(20)
                .withWhere({
...
exports.InterviewVectorStore = InterviewVectorStore;
