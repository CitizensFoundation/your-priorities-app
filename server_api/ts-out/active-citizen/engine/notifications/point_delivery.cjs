...
module.exports = function (notification, user, callback) {
    if (notification &&
        user &&
        notification.AcActivities &&
        notification.AcActivities.length > 0) {
        var contentName = "";
        if (notification.AcActivities[0].Post) {
            contentName = notification.AcActivities[0].Post.name;
        }
        else if (notification.AcActivities[0].Point.Group &&
        ...
        if (notification.type == "notification.point.new") {
            filterNotificationForDelivery(notification, user, "point_activity", {
                translateToken: "notification.email.newPointOnMyPoint",
                contentName: contentName,
            }, callback);
        }
        else if (notification.type == "notification.point.newsStory") {
            filterNotificationForDelivery(notification, user, "point_news_story", {
                translateToken: "notification.email.newsStory",
                contentName: contentName,
            }, callback);
        }
        else if (notification.type == "notification.point.comment") {
            filterNotificationForDelivery(notification, user, "point_comment", {
                translateToken: "notification.email.comment",
                contentName: contentName,
            }, callback);
        }
        else if (notification.type == "notification.point.quality") {
            var subjectTranslateToken = notification.AcActivities[0].type.indexOf("activity.point.helpful") > -1
                ? "notification.email.pointHelpful"
                : "notification.email.pointUnhelpful";
