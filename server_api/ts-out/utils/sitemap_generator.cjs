            ...
            async.series([
                    ...
                    models.Group.findAll({
                        ...
                        _.forEach(groups, function (group) {
                            ...
                            if (group.Community.hostname &&
                                wildCardDomainNames.indexOf(domainName) > -1) {
                                links.push({
                                    url: getCommunityURL(group.Community.hostname, domainName, path),
                                });
                            }
                            else {
                                links.push({ url: path });
                ...
                if (error) {
                    ...
                    try {
                        ...
                        streamToPromise(Readable.from(links).pipe(sitemapStream)).then((xml) => {
                            xml = xml.toString();
                            req.redisClient.setEx(redisKey, process.env.SITEMAP_CACHE_TTL
