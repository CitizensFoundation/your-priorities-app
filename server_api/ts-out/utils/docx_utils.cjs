...
const getRatingHeaders = require('./export_utils.cjs').getRatingHeaders;
const getContactData = require('./export_utils.cjs').getContactData;
const getAttachmentData = require('./export_utils.cjs').getAttachmentData;
const getMediaTranscripts = require('./export_utils.cjs').getMediaTranscripts;
const getPostRatings = require('./export_utils.cjs').getPostRatings;
const getPostUrl = require('./export_utils.cjs').getPostUrl;
const getLocation = require('./export_utils.cjs').getLocation;
const getCategory = require('./export_utils.cjs').getCategory;
const getUserEmail = require('./export_utils.cjs').getUserEmail;
const clean = require('./export_utils.cjs').clean;
...
const setDescriptions = (group, post, builtPost, children) => {
    if (group && group.configuration && group.configuration.structuredQuestions && group.configuration.structuredQuestions !== "") {
        ...
        children.push(new Paragraph(builtPost.translatedDescription ? builtPost.translatedDescription : post.description));
    }
};
...
const addPointTranslationIfNeeded = (post, point, children) => {
    if (post.translatedPoints && post.translatedPoints[point.id]) {
        children.push(new Paragraph(post.translatedPoints[point.id]));
    }
    else {
        children.push(new Paragraph(point.content));
    }
    children.push(new Paragraph(""));
};
const addPostToDoc = (doc, post, group) => {
    ...
    children.push(new Paragraph(""), new Paragraph("Original locale: " + post.realPost.language), new Paragraph("URL: " + post.url), new Paragraph(""), new Paragraph("User email: " + post.userEmail), new Paragraph("User name: " + post.userName), new Paragraph(""), new Paragraph("Endorsements up: " + post.endorsementsUp), new Paragraph("Endorsements down: " + post.endorsementsDown), new Paragraph("Counter points: " + post.counterPoints), new Paragraph(""));
    ...
    if (post.mediaTranscripts && post.mediaTranscripts.length > 4) {
        children.push(new Paragraph(""));
        children.push(new Paragraph("Media transcripts: \n" + post.mediaTranscripts));
    }
