...
const createS3Client = (options = {}) => {
    const useAccelerateEndpoint = options.useAccelerateEndpoint === undefined
        ? undefined
    ...
    const forcePathStyle = useAccelerateEndpoint
        ? undefined
    ...
    const config = {
        region: options.region ||
            ...
            "us-east-1",
        requestChecksumCalculation: options.requestChecksumCalculation || "WHEN_REQUIRED",
    };
    if (credentials) {
...
const extractBucketRegion = (result) => normalizeBucketRegion(result?.BucketRegion ||
    getHeaderValue(result?.$metadata?.httpHeaders, "x-amz-bucket-region") ||
    getHeaderValue(result?.$response?.headers, "x-amz-bucket-region") ||
    getHeaderValue(result?.$response?.httpResponse?.headers, "x-amz-bucket-region"));
...
const createS3ClientForBucket = async (bucket, options = {}) => createS3Client({
    ...options,
...
const getPresignedPutObjectUrlForBucket = async (params, clientOptions = {}, presignOptions = {}) => {
...
const getPresignedGetObjectUrlForBucket = async (params, clientOptions = {}, presignOptions = {}) => {
    const client = await createS3ClientForBucket(params.Bucket, clientOptions);
