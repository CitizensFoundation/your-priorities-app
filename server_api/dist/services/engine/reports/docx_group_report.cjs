...
const fs = require("fs");
const { Document, Packer, Paragraph, Table, TableCell, UnderlineType, HeadingLevel, AlignmentType, ImageRun, TableRow, } = docx;
...
const updateJobStatusIfNeeded = require("./common_utils.cjs").updateJobStatusIfNeeded;
...
const setDescriptions = (group, post, builtPost, children) => {
    if (group &&
        ...
        children.push(new Paragraph(cleanText(builtPost.translatedDescription
...
const addPointTranslationIfNeeded = (group, post, point, children) => {
    ...
    if (point.public_data &&
        ...
        children.push(new Paragraph(""));
        children.push(new Paragraph(group.translatedCustomAdminCommentsTitle ||
...
const addPostToDoc = (doc, post, group) => {
    ...
    let pointForText = group.translatedAlternativePointForHeader ||
    ...
    async.series([
        ...
        (seriesCallback) => {
            ...
            children.push(new Paragraph({
                ...
                heading: HeadingLevel.HEADING_1,
            }), new Paragraph(group.translatedObjectives
            ...
            if (group.targetTranslationLanguage) {
                children.push(new Paragraph(""), new Paragraph({
                    text: "Automatically machine translated to locale: " +
