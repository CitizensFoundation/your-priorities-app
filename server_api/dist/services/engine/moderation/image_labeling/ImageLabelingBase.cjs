...
const hasImageExtension = (imageUrl) => /\.(gif|jpe?g|png|webp)$/i.test(new URL(imageUrl).pathname);
...
const downloadImageFile = async (imageUrl) => {
    ...
    const fileNameWithPath = path.join("/tmp", `vision-${Date.now()}-${crypto.randomBytes(8).toString("hex")}${extension}`);
    let response;
    try {
                    ...
                    if (video.meta &&
                        ...
                        if (video.VideoImages.length > 0) {
                            ...
                            for (let image of searchImages) {
                                const imageFormat = JSON.parse(image.formats);
                                const reviewResult = await this.reviewAndLabelUrl(imageFormat[imageFormat.length - 1], "Video", image.id);
                                if (!reviewResult) {
                                    continue;
                                }
