...
module.exports = {
    up: async (queryInterface, Sequelize) => {
        return [
            ...
            await queryInterface.sequelize.query("UPDATE points SET status = 'published' WHERE status='active'"),
            ...
            await queryInterface.sequelize.query("UPDATE posts SET status = 'published' WHERE status='inactive'"),
            await queryInterface.addIndex('users', ['id', 'deleted', 'status']),
            await queryInterface.sequelize.query("UPDATE users SET status = 'active' WHERE status='pending'")
        ];
    },
