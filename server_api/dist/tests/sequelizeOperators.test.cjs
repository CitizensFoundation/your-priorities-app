...
const legacyOperatorPattern = /["']?\$(?:gt|gte|lt|lte|in|and|or|eq|ne|is|not|between|notBetween|like|contains|any)["']?\s*:/g;
...
const loadCommonJsModule = (modulePath, mocks) => {
    ...
    const mockedRequire = (request) => Object.hasOwn(mocks, request) ? mocks[request] : baseRequire(request);
    const compiled = vm.runInThisContext(Module.wrap(source), {
        filename: modulePath,
    });
    compiled(loadedModule.exports, mockedRequire, loadedModule, modulePath, path.dirname(modulePath));
...
test("news-feed query composition preserves Sequelize symbols", () => {
    const newsFeedsUtilsPath = path.resolve(__dirname, "../services/engine/news_feeds/news_feeds_utils.cjs");
...
test("notification pagination preserves date operator symbols", async () => {
    const notificationsControllerPath = path.resolve(__dirname, "../services/controllers/notifications.cjs");
    ...
    const runRequest = (query) => new Promise((resolve, reject) => {
        getNotifications({ query, user: { id: 42 } }, {
            send: resolve,
            sendStatus(status) {
    ...
    assert.equal(firstQueryWhere[0].updated_at[Op.lt].toISOString(), "2025-01-01T00:00:00.000Z");
    assert.equal(firstQueryWhere[1].updated_at[Op.gt].toISOString(), "2024-01-01T00:00:00.000Z");
});
