...
const { UrlPreviewError, createPinnedLookup, fetchPublicHtml, getUrlPreview, isPublicAddress, parseAndValidateUrl, sanitizeUrlForLogging, } = require("../services/urlPreviewService.cjs");
const { DEFAULT_MAX_REQUESTS, DEFAULT_WINDOW_MS, createUrlPreviewRateLimitOptions, sendRateLimitResponse, urlPreviewRateLimitKey, } = require("../services/urlPreviewRateLimiter.cjs");
const PUBLIC_IPV4 = "93.184.216.34";
const publicResolver = async () => [
    { address: PUBLIC_IPV4, family: 4 },
];
const htmlResponse = (html = "<html><head><title>Example</title></head></html>") => new Response(html, {
    status: 200,
...
test("URL validation permits only credential-free HTTP(S) on ports 80 and 443", () => {
    assert.equal(parseAndValidateUrl("https://example.com/path#fragment").toString(), "https://example.com/path");
    assert.equal(parseAndValidateUrl("http://example.com:443/path").port, "443");
    for (const url of [
        "",
        "example.com",
        ...
        assert.throws(() => parseAndValidateUrl(url), (error) => error.code === "INVALID_PREVIEW_URL", url);
    }
});
...
test("literal private and alternative numeric loopback URLs are blocked before fetch", async () => {
    ...
    for (const url of [
        ...
        await assertRejectCode(fetchPublicHtml(url, { fetchFn }), "PREVIEW_URL_BLOCKED");
    }
    assert.equal(fetchCalls, 0);
});
test("a hostname with any non-public DNS answer is blocked", async () => {
    let fetchCalls = 0;
    await assertRejectCode(fetchPublicHtml("https://mixed.example/", {
        resolver: async () => [
            { address: PUBLIC_IPV4, family: 4 },
...
test("pinned lookup never performs or substitutes a second DNS resolution", async () => {
    ...
    const all = await new Promise((resolve, reject) => {
        lookup("attacker-controlled.example", { all: true }, (error, addresses) => error ? reject(error) : resolve(addresses));
    });
    assert.deepEqual(all, [{ address: PUBLIC_IPV4, family: 4 }]);
    const single = await new Promise((resolve, reject) => {
        lookup("attacker-controlled.example", {}, (error, address, family) => error ? reject(error) : resolve({ address, family }));
    });
...
test("public redirects are followed manually and revalidated per hop", async () => {
    ...
    assert.equal(dispatchers.records.every((record) => record.dispatcher.closed), true);
});
test("a public URL redirecting to a private host is blocked before the second fetch", async () => {
    let fetchCalls = 0;
    await assertRejectCode(fetchPublicHtml("https://start.example/", {
        resolver: async (hostname) => hostname === "start.example"
            ? [{ address: PUBLIC_IPV4, family: 4 }]
...
test("redirect limits are enforced", async () => {
    let fetchCalls = 0;
    await assertRejectCode(fetchPublicHtml("https://loop.example/", {
        maxRedirects: 2,
...
test("response content type, status, and byte limits are enforced", async () => {
    ...
    await assertRejectCode(fetchPublicHtml("https://public.example/json", {
        ...baseOptions,
    ...
    await assertRejectCode(fetchPublicHtml("https://public.example/missing", {
        ...baseOptions,
        fetchFn: async () => new Response("missing", {
            status: 404,
            headers: { "content-type": "text/html" },
        }),
    }), "PREVIEW_FETCH_FAILED");
    await assertRejectCode(fetchPublicHtml("https://public.example/large", {
        ...baseOptions,
        maxBytes: 4,
        fetchFn: async () => new Response("12345", {
            status: 200,
            headers: { "content-type": "text/html" },
        }),
...
test("the total request timeout maps to a typed gateway timeout", async () => {
    await assertRejectCode(fetchPublicHtml("https://slow.example/", {
        timeoutMs: 5,
        resolver: publicResolver,
        dispatcherFactory: createDispatcherRecorder().factory,
...
test("HTML decoding honors meta and XML character encoding declarations", async () => {
    ...
    for (const [index, declaration] of declarations.entries()) {
        const encodedHtml = iconv.encode(`${declaration}<title>${expectedTitle}</title></head></html>`, "windows-1252");
        const result = await fetchPublicHtml(`https://public.example/legacy-${index}`, {
            resolver: publicResolver,
            dispatcherFactory: createDispatcherRecorder().factory,
            fetchFn: async () => new Response(encodedHtml, {
                headers: { "content-type": "text/html" },
            }),
        });
...
test("HTTP character encoding takes precedence over HTML metadata", async () => {
    const expectedTitle = "Hæ – Café";
    const encodedHtml = iconv.encode(`<html><head><meta charset="utf-8"><title>${expectedTitle}</title></head></html>`, "windows-1252");
...
test("Open Graph parsing receives HTML only and preserves the success metadata", async () => {
    ...
    const preview = await getUrlPreview("https://public.example/article#section", {
        resolver: publicResolver,
        dispatcherFactory: createDispatcherRecorder().factory,
        fetchFn: async () => htmlResponse("<html><head><title>Secret-free title</title></head><body>raw body</body></html>"),
...
test("log sanitization drops paths, queries, fragments, and malformed input", () => {
    assert.equal(sanitizeUrlForLogging("https://example.com/path?secret=value#fragment"), "https://example.com");
...
test("URL preview rate limiting is Redis-backed and keyed by authenticated user", () => {
    ...
    assert.match(options.keyGenerator({ ip: "2001:db8::1", socket: {} }), /^ip:/);
    assert.ok(options.store);
});
test("URL preview rate limiting uses safe defaults and a typed 429 response", () => {
    const options = createUrlPreviewRateLimitOptions({ sendCommand: async () => 1 }, {
