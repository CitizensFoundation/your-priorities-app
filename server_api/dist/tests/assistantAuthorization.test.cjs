...
const servicePath = path.resolve(__dirname, "../agents/services/assistantAccessService.ts");
const controllerPath = path.resolve(__dirname, "../agents/controllers/assistantsController.ts");
const workflowManagerPath = path.resolve(__dirname, "../agents/managers/workflowConversationManager.ts");
const voiceAssistantBasePath = path.resolve(__dirname, "../agents/assistants/baseAssistantWithVoice.ts");
const loadAccessService = ({ findRun, findSubscription } = {}) => {
    ...
    compiled(module.exports, mockedRequire, module, servicePath, path.dirname(servicePath));
...
const loadWorkflowConversationManager = (findOne) => {
    ...
    compiled(module.exports, mockedRequire, module, workflowManagerPath, path.dirname(workflowManagerPath));
    return module.exports;
};
...
test("validates positive identifiers, UUIDs, and domain-isolated memory keys", () => {
    const { buildAssistantMemoryRedisKey, isUuid, parsePositiveInteger, } = loadAccessService();
...
test("scopes runs by owner and rejects mismatched group or agent IDs", async () => {
    ...
    assert.equal(await service.getOwnedRun({ userId: 9, runId: 5, groupId: 99 }), null);
    assert.equal(await service.getOwnedRun({
        userId: 9,
        runId: 5,
        groupId: 20,
        agentId: 31,
...
test("workflow conversation updates are scoped to the owning user", async () => {
    ...
    const { WorkflowConversationManager } = loadWorkflowConversationManager(async (options) => {
        query = options;
        return conversation;
    });
    const result = await new WorkflowConversationManager().connectToWorkflowConversation(8, 42, { connected: true });
...
test("route registration guards sensitive endpoints and removes obsolete ones", () => {
    ...
    for (const [route, handler] of guardedRoutes) {
        ...
        assert.match(source.slice(start, end), /auth\.isLoggedInNoAnonymousCheck/, `missing login guard for ${route}`);
    }
...
test("assistant instances are cleaned up on disconnect and initialization failure", () => {
    ...
    assert.match(controllerSource, /void assistant\.conversation\(chatLog\)\.catch/);
    assert.match(controllerSource, /cleanupAssistantForMemory\(assistantMemoryKey, assistantInstance\)/);
