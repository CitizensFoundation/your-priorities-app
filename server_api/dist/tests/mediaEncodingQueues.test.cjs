...
test("media encoding queues are created lazily and reused", async (t) => {
    ...
    const originals = new Map(pathsToRestore.map((modulePath) => [modulePath, require.cache[modulePath]]));
    ...
    const startAudioJob = () => new Promise((resolve, reject) => {
        Audio.startYrpriEncoderTranscodingJob({ meta: { fileKey: "audio.mp4", maxDuration: 10 } }, (error, result) => (error ? reject(error) : resolve(result)));
    });
    await startAudioJob();
    await startAudioJob();
    ...
    assert.deepEqual(addedJobs.map(({ queue, name }) => ({ queue, name })), [
