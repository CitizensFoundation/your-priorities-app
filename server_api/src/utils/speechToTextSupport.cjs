const getSpeechToTextSupportConfig = () => {
  ...
  return {
    hasGoogleCredentials:
      typeof googleCredentials === "string" && googleCredentials.trim().length > 0,
    hasTranscodingFlacBucket:
      typeof transcodingFlacBucket === "string" &&
      transcodingFlacBucket.trim().length > 0,
    googleCredentialsLength:
      typeof googleCredentials === "string" ? googleCredentials.length : 0,
    transcodingFlacBucket: transcodingFlacBucket || null,
    transcodingFlacBucketLength:
      typeof transcodingFlacBucket === "string" ? transcodingFlacBucket.length : 0,
  };
};
