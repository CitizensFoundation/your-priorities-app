...
export const MAX_MARKDOWN_REPORT_LENGTH = 1_000_000;
...
const stripIllegalXmlCharacters = (value: string): string =>
  value.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/g, "");
...
const normalizeText = (value: string): string =>
  stripIllegalXmlCharacters(decodeHtmlEntities(value));
...
const renderInlineTokens = (
  ...
  for (const token of tokens) {
    switch (token.type) {
      ...
      case "escape":
        children.push(...textRuns((token as Tokens.Escape).text, style));
        break;
      case "strong": {
        const strong = token as Tokens.Strong;
        children.push(
          ...renderInlineTokens(strong.tokens, { ...style, bold: true }, depth + 1)
        );
        break;
      }
      case "em": {
        ...
        children.push(
          ...renderInlineTokens(emphasis.tokens, { ...style, italics: true }, depth + 1)
        );
        break;
      }
      case "del": {
        ...
        children.push(
          ...renderInlineTokens(deleted.tokens, { ...style, strike: true }, depth + 1)
        );
        break;
      }
      case "codespan":
        children.push(...textRuns((token as Tokens.Codespan).text, { ...style, code: true }));
        break;
      case "br":
...
const contextParagraphOptions = (context: BlockContext): IParagraphOptions => {
...
const inlineChildrenForBlockToken = (token: Token): ParagraphChild[] => {
...
const renderList = (list: Tokens.List, context: BlockContext): FileChild[] => {
  ...
  list.items.forEach((item, itemIndex) => {
    ...
    for (const itemToken of item.tokens) {
      if (itemToken.type === "list") {
        children.push(
          ...renderList(itemToken as Tokens.List, {
            ...context,
...
const renderTableCell = (cell: Tokens.TableCell): TableCell =>
  new TableCell({
    children: [
      new Paragraph({
...
const renderTable = (table: Tokens.Table): Table =>
  new Table({
    rows: [
      ...
      ...table.rows.map(
        (row) => new TableRow({ children: row.map(renderTableCell) })
      ),
    ],
...
const renderCodeBlock = (code: Tokens.Code, context: BlockContext): Paragraph =>
  new Paragraph({
...
const renderBlocks = (
  tokens: readonly Token[],
  context: BlockContext = { listDepth: 0, quoteDepth: 0 }
): FileChild[] => {
  const children: FileChild[] = [];
  ...
  for (const token of tokens) {
    switch (token.type) {
      ...
      case "text":
        ...
        break;
      case "code":
        children.push(renderCodeBlock(token as Tokens.Code, context));
        break;
      case "blockquote": {
        const blockquote = token as Tokens.Blockquote;
        children.push(
          ...renderBlocks(blockquote.tokens, {
            ...context,
      ...
      case "list":
        children.push(...renderList(token as Tokens.List, context));
        break;
      case "table":
      ...
      case "hr":
        ...
        break;
      case "html": {
        const text = htmlToPlainText((token as Tokens.HTML).text).trim();
        if (text) {
          children.push(
            new Paragraph({
              ...contextParagraphOptions(context),
...
export const markdownToDocx = async (
  ...
  if (markdown.length > MAX_MARKDOWN_REPORT_LENGTH) {
    throw new RangeError(
      `Markdown report exceeds ${MAX_MARKDOWN_REPORT_LENGTH} characters`
    );
  }
