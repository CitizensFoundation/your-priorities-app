...
const hasGroupAdmin = async (group, user) => {
  ...
  if (isSameId(group.user_id, user.id)) return true;
...
const hasCommunityAdmin = async (community, user) => {
  ...
  if (isSameId(community.user_id, user.id)) return true;
...
const hasDomainAdmin = async (domain, user) => {
  ...
  if (isSameId(domain.user_id, user.id)) return true;
...
const canGenerateForContext = async ({
  collection,
  collectionType,
  generationContext,
  imageType,
  ...
  if (
    generationContext === GENERATION_CONTEXTS.AOI_ICON_PUBLIC ||
    generationContext === GENERATION_CONTEXTS.AOI_ICON_ADMIN
  ) {
    if (imageType !== "icon") return false;
  ...
  switch (generationContext) {
    ...
    case GENERATION_CONTEXTS.AOI_ICON_ADMIN:
      if (collectionType === "group") return await hasGroupAdmin(collection, user);
      if (collectionType === "community") {
        return await hasCommunityAdmin(collection, user);
      }
      if (collectionType === "domain") return await hasDomainAdmin(collection, user);
      return false;
    case GENERATION_CONTEXTS.REGULAR_AI_IMAGE:
      if (collectionType === "group") {
        return (
          (await hasGroupAdmin(collection, user)) ||
          Boolean(collection.configuration?.allowGenerativeImages)
        );
      }
      if (collectionType === "community") {
        return (
          (await hasCommunityAdmin(collection, user)) ||
          !collection.only_admins_can_create_groups
        );
      }
      if (collectionType === "domain") {
        return (
          (await hasDomainAdmin(collection, user)) ||
...
const validateImageGenerationStartRequest = async (
  req,
...
const canPollImageGenerationJob = (req, job) =>
  Boolean(req.user && job?.internal_data?.userId === req.user.id);
