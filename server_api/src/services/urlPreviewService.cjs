...
const sanitizeUrlForLogging = (urlIn) => {
  try {
    if (typeof urlIn !== "string") return "[invalid-url]";
...
const createPinnedDispatcher = ({ addresses, timeoutMs, maxBytes }) =>
  new Agent({
    connect: {
...
const isTimeoutError = (error, signal) =>
  signal?.aborted ||
...
const isResponseTooLargeError = (error) =>
  hasErrorCode(error, responseTooLargeErrorCodes);
...
const fetchPublicHtml = async (urlIn, options = {}) => {
  ...
  while (true) {
    ...
    try {
      ...
      if (error instanceof UrlPreviewError) throw error;
