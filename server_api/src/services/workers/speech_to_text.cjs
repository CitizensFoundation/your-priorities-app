...
const getTranscriptRetryDelayMs = (retryCount) =>
  Math.min((retryCount + 1) * 10000, 60000);
...
const getTempFlacFile = (sourceFileName) => {
  const fileName =
    sourceFileName || `speech-${Date.now()}-${crypto.randomBytes(8).toString("hex")}.flac`;
          ...
          if (error) {
            ...
            if (
              ...
              const transcriptError =
                error === "Could not download file" ? "Found no audio file" : error;
