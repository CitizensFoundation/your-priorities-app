...
export class WebSocketsManager {
  ...
  private maxRedisReconnectAttempts = 5;
  private redisReconnectDelay = 2000; // milliseconds

  // Unique ID to identify this server instance
  ...
  constructor(
    ...
    this.serverId =
      process.env.NODE_ENV === "development"
        ...
        : `server-${Math.random()}`;
    console.log(
      "WebSockets: Starting WebSocketsManager on serverId:",
      this.serverId
    );
  }
  ...
  async listen() {
    ...
    this.ws.on("connection", async (ws: WebSocket, req: IncomingMessage) => {
      ...
      if (!claimed) {
        ...
        console.log(
          `WebSockets: Server ${this.serverId} could not claim clientId ${clientId} immediately.`
        );
      } else {
        console.log(
          `WebSockets: Server ${this.serverId} claimed clientId ${clientId}.`
        );
      }
      ...
      console.log(
        `WebSockets: New WebSocket connection on serverId ${this.serverId}: clientId ${clientId}`
      );
      ...
      ws.on("message", (messageData: Buffer) => {
        ...
        if (!this.wsClients.has(clientId)) {
          ...
          try {
            ...
            console.log(
              `WebSockets: Received non-JSON message from client ${clientId}:`,
          ...
          this.pub
            ...
            .then((reply) => {
              console.log(
                `WebSockets: Message published to ypWebsocketChannel: ${reply}`
              );
            })
            .catch((err) => {
      ...
      ws.on("close", async () => {
        ...
        console.log(
          `WebSockets: WebSocket closed: clientId ${clientId} on server ${this.serverId}`
        );
        ...
        const currentOwner = await this.redisClient.get(
        ...
        if (currentOwner === this.serverId) {
          ...
          console.log(
            `WebSockets: Server ${this.serverId} removed ownership for clientId ${clientId} from Redis.`
          );
        }
      });
  ...
  // -------------------------------
  private async tryClaimClientId(clientId: string): Promise<boolean> {
    ...
    if (result === "OK") {
      ...
      console.log(
        `WebSockets: Unable to claim clientId ${clientId}, already owned by ${currentOwner}`
      );
      return false;
    }
  }
  ...
  // -------------------------------
  private async requestReleaseClientId(clientId: string) {
    const currentOwner = await this.redisClient.get(`clientOwner:${clientId}`);
    if (!currentOwner) return; // No one actually owns it, maybe it expired?
    ...
    console.log(
      `WebSockets: Requesting server ${currentOwner} to release clientId ${clientId}`
    );
    const msg = {
  ...
  private async setupPubSub() {
    ...
    this.sub.subscribe("ypWebsocketChannel", (message, channel) => {
      try {
        ...
        console.log(
          `WebSockets: Received from Redis on ${channel}: ${JSON.stringify(
            parsed
          )}`
        );
        ...
        switch (action) {
          case "directMessage":
            ...
            if (ws) {
              try {
                ...
                console.error(
                  `WebSockets: Error sending direct message to ${clientId}:`,
                  err
                );
              }
            } else {
              ...
              console.warn(
                `WebSockets: No WebSocket found locally for clientId ${clientId}`
              );
              this.wsClients.delete(clientId);
            }
            break;
          ...
          default:
            console.warn(
              `WebSockets: Unknown action '${action}' received from Redis.`
            );
        }
      } catch (err) {
      ...
      try {
        ...
        if (action === "releaseClientId") {
          ...
          if (currentOwner === this.serverId && this.serverId === oldOwner) {
            ...
            console.log(
              `WebSockets: Server ${this.serverId} is releasing clientId ${clientId} (requested by ${fromServer}).`
            );
            ...
            await this.redisClient.del(`clientOwner:${clientId}`);
            console.log(
              `WebSockets: Server ${this.serverId} removed ownership for clientId ${clientId}.`
            );
          } else {
            // We don't own it or the ownership mismatch
            // Possibly ignore or log
            console.log(
              `WebSockets: releaseClientId ignored by ${this.serverId}. currentOwner=${currentOwner}, oldOwner=${oldOwner}`
            );
          }
        }
      } catch (err) {
  ...
  private async connectRedisClient(
    ...
    while (attempts < this.maxRedisReconnectAttempts) {
      try {
        ...
        console.error(
          `WebSockets: ${clientName} connection attempt ${attempts} failed:`,
          err
        );
        await new Promise((resolve) =>
          setTimeout(resolve, this.redisReconnectDelay)
        );
      }
    }
    throw new Error(
      `WebSockets: ${clientName} failed to connect after ${this.maxRedisReconnectAttempts} attempts`
    );
  }
   ...
   */
  private async handleRedisError(client: RedisClientType) {
    try {
      client.disconnect();
    } catch (err) {
      console.error(
        "WebSockets: Error disconnecting Redis client during error handling:",
        err
      );
    }
   ...
   */
  public async shutdownPubSub(): Promise<void> {
    try {
