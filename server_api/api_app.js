...
if (!process.env.S3_BUCKET) {
  ...
  const errorText =
    "No S3_BUCKET is set, image uploads and report downloads will not work";
  log.error(errorText);
...
app.use(function setupStaticPath(req, res, next) {
  ...
  express.static(staticPath, { index: staticIndex, dotfiles: "allow" })(
    req,
    res,
    next
  );
  ...
  user
    ...
    .then(() => {
      models.AcActivity.createActivity(
        ...
        function (error) {
          if (error) log.error("Error in create activity", { error });
          done();
        }
      );
    })
...
passport.serializeUser(function userSerialize(req, profile, done) {
  ...
  if (profile.provider && profile.provider === "facebook") {
    models.User.serializeFacebookUser(
      profile,
      req.ypDomain,
      function (error, user) {
        if (error) {
...
app.use(
  "/promotion",
  express.static(path.join(__dirname, "../webApps/old/promotion_app/dist"))
);
app.use(
  "/promotion/domain/*",
  express.static(path.join(__dirname, "../webApps/old/promotion_app/dist"))
...
app.use(
  ...
  express.static(path.join(__dirname, "../webApps/old/promotion_app/dist/locales/en"))
);
app.use(
  "/promotion/locales/is/*",
  express.static(path.join(__dirname, "../webApps/old/promotion_app/dist/locales/is"))
);
...
app.use(
  "/analytics/",
  express.static(path.join(__dirname, "../webApps/old/analytics_app/dist"))
);
app.use(
  "/analytics/domain/*",
  express.static(path.join(__dirname, "../webApps/old/analytics_app/dist"))
...
app.use(
  "/admin/",
  express.static(path.join(__dirname, "../webApps/old/translationApp/dist"))
);
app.use(
  "/admin/domain/*",
  express.static(path.join(__dirname, "../webApps/old/translationApp/dist"))
...
initializeEsControllers().catch((error) =>
  console.error("Error initializing es controllers:", error)
);
    ...
    function () {
      log.info(
        `Your Priorities Platform API Server listening on port ${
          process.env.YOUR_PRIORITIES_LISTEN_HOST
        }:${app.get("port")} on ${process.env.NODE_ENV}`
      );
    }
  );
} else {
