          ...
          <div class="surveyContainer">
            <div id="surveyContainer">
              <template is="dom-repeat" items="[[structuredQuestions]]" as="question" initial-count="5">
                <yp-structured-question-edit index="[[index]]" id="structuredQuestionContainer_[[index]]" structured-answers="[[initiallyLoadedAnswers]]" on-changed="_saveState" question="[[question]]" is-last-rating="[[_isLastRating(index)]]" is-first-rating="[[_isFirstRating(index)]]" on-hide-to-question="_hideToQuestion">
                </yp-structured-question-edit>
              </template>
            </div>
    ...
    <template is="dom-if" if="[[surveyCompleted]]">
      <div class="surveyCompleted layout vertical center-center">
        ...
        <template is="dom-if" if="[[surveyGroup.configuration.customThankYouTextNewPosts]]">
          <yp-magic-text id="customThankYouTextNewPostsId" hidden="" content-id="[[surveyGroup.id]]" text-only="" content="[[surveyGroup.configuration.customThankYouTextNewPosts]]" content-language="[[surveyGroup.language]]" text-type="customThankYouTextNewPosts"></yp-magic-text>
        </template>
        <template is="dom-if" if="[[!surveyGroup.configuration.customThankYouTextNewPosts]]">
          [[t('thankYouForCompletingTheSurvey')]]
        </template>
