...
class YpSurveyGroupLit extends YpBaseElement {
  ...
  render() {  
    ...
    ${!this.surveyCompleted ? html`
      ${this.structuredQuestions ? html` 
        <div class="layout vertical center-center">
          <div class="surveyContainer">
            <div id="surveyContainer">
              ${this.structuredQuestions.map(question=> html`
                <yp-structured-question-edit index="${this.index}" id="structuredQuestionContainer_${this.index}" .structuredAnswers="${this.initiallyLoadedAnswers}" @changed="${this._saveState}" .question="${this.question}" is-last-rating="${this._isLastRating(index)}" is-first-rating="${this._isFirstRating(index)}" @hide-to-question="${this._hideToQuestion}">
                </yp-structured-question-edit>
              `)}
            </div>
            <div class="layout horizontal center-center submitSpinnerContainer">
              <yp-ajax .largeSpinner id="submitAjax" .method="POST" .ironMethod="POST" @response="${this._surveySubmitResponse}"></yp-ajax>
            </div>
            <div class="layout horizontal center-center" hidden="${this.submitHidden}">
              <paper-button id="submitButton" raised @tap="${this._submit}">${this.t('submitSurvey')}</paper-button>
            </div>
          </div>
        </div>
      ` : html``}
    ` : html`
      <div class="surveyCompleted layout vertical center-center">
        ...
        ${this.surveyGroup.configuration.customThankYouTextNewPosts ? html` 
          <yp-magic-text id="customThankYouTextNewPostsId" hidden contentId="${this.surveyGroup.id}" .textOnly content="${this.surveyGroup.configuration.customThankYouTextNewPosts}" .contentLanguage="${this.surveyGroup.language}" .textType="customThankYouTextNewPosts"></yp-magic-text>
        `: html`
          ${this.t('thankYouForCompletingTheSurvey')} 
        `} 
      </div> 
    ...
    <div class="layout horizontal center-center">
      <yp-ajax .largeSpinner="" id="ajax" @response="${this._surveyGroupResponse}"></yp-ajax>
      <yp-ajax hidden id="pagesAjax" @response="${this._pagesResponse}"></yp-ajax>
    </div>
    `
  }
...
*/
  ...
  _surveyGroupResponse(event, detail) {
    ...
    this.structuredQuestions = detail.response.surveyGroup.configuration.structuredQuestionsJson;
...
window.customElements.define('yp-survey-group-lit', YpSurveyGroupLit);