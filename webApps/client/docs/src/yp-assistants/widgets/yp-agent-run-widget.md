...
| runStatus         | YpAgentProductRunStatus       | The current status of the agent run.                                        |
...
| parseWorkflow                 | None                                                                       | void         | Parses the workflow configuration and updates agent and workflow group IDs. |
...
| startStatusUpdates            | None                                                                       | void         | Starts periodic updates of the agent status.                                |
| stopStatusUpdates             | None                                                                       | void         | Stops periodic updates of the agent status.                                 |
...
| getStepClass                  | index: number                                                              | string       | Returns the CSS class for a workflow step based on its status.              |
| openGroup                     | groupId: number                                                            | void         | Opens a new window to the specified group.                                  |
...
| get isRunning                 | None                                                                       | boolean      | Checks if the agent is currently running.                                   |
| get isWaitingOnUser           | None                                                                       | boolean      | Checks if the agent is waiting on user input.                               |
| get isCompleted               | None                                                                       | boolean      | Checks if the agent run is completed.                                       |
| renderAgentRunningStatus      | None                                                                       | TemplateResult | Renders the agent running status section.                                   |
| get runStatusForButton        | None                                                                       | string       | Returns the label for the start/stop button based on the run status.        |
...
| get groupId                   | None                                                                       | number       | Returns the group ID of the current workflow step.                          |
| sendEmailInvitesForAnons      | None                                                                       | Promise<void>| Sends email invites for anonymous users.                                    |
| viewList                      | None                                                                       | void         | Opens a new window to view the list of users.                               |
| openMarkdownReport            | None                                                                       | void         | Opens the markdown report.                                                  |
| renderCompleted               | None                                                                       | TemplateResult | Renders the completed state view.                                           |
| renderWaitingOnUser           | None                                                                       | TemplateResult | Renders the waiting on user state view.                                     |
